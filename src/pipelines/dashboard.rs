...
pub fn run(config: &Config, db: &Database, args: &DashboardArgs) -> Result<()> {
    ...
    let html = render_dashboard(&history, &config.branding);
...
fn render_dashboard(history: &[MetricSnapshot], branding: &crate::config::BrandingConfig) -> String {
    let json_data = serde_json::to_string(history).unwrap_or_else(|_| "[]".to_string());
    let bot_name = html_escape(&branding.name);
    let bot_url = html_escape(&branding.url);
...
<div class="footer">Generated by <a href="{bot_url}">{bot_name}</a></div>
