...
impl AzureDevOpsProvider {
    ...
    fn git_api(&self, path: &str) -> String {
        format!(
            ...
            self.base_url, self.org, self.project, self.repo_name, path
        )
    }

    fn wit_api(&self, path: &str) -> String {
        format!(
            "{}/{}/{}/_apis/wit/{}?api-version=7.1",
            self.base_url, self.org, self.project, path
        )
    }
...
impl GitProvider for AzureDevOpsProvider {
    ...
    fn repo_slug(&self) -> String {
        format!("{}/{}/{}", self.org, self.project, self.repo_name)
    }
            ...
            if let Some(id) = wi["id"].as_u64() {
                ...
                if let Ok(item) = self.get(&url).await {
                    ...
                    issues.push(Issue {
                        ...
                        labels: vec![fields["System.WorkItemType"]
                            .as_str()
                            .unwrap_or("")
                            .to_string()],
                        author: fields["System.CreatedBy"]["uniqueName"]
                            .as_str()
                            .map(String::from),
    ...
    async fn comment_issue(&self, number: u64, body: &str, marker: &str) -> Result<()> {
        if let Some(comment_id) = self.find_comment_with_marker(number, marker).await? {
            ...
            self.patch(&url, &serde_json::json!({ "text": body }))
                .await?;
        } else {
            let url = self.wit_api(&format!("workitems/{number}/comments"));
            self.post(&url, &serde_json::json!({ "text": body }))
                .await?;
        }
        Ok(())
    }
        ...
        Ok(prs
            ...
            .map(|pr| PullRequest {
                ...
                base_sha: pr["lastMergeTargetCommit"]["commitId"]
                    .as_str()
                    .map(String::from),
                head_ref: pr["sourceRefName"]
                    .as_str()
                    .map(|s| s.replace("refs/heads/", "")),
                base_ref: pr["targetRefName"]
                    .as_str()
                    .map(|s| s.replace("refs/heads/", "")),
    ...
    async fn fetch_pr_diff(&self, number: u64) -> Result<String> {
        ...
        if let Some(last) = iterations.last() {
            if let Some(id) = last["id"].as_u64() {
                let changes_url =
                    self.git_api(&format!("pullrequests/{number}/iterations/{id}/changes"));
    ...
    async fn create_pr(
        ...
        let resp = self
            ...
            .await?;
        Ok(resp["pullRequestId"]
            .as_u64()
            .context("Missing pullRequestId")?)
    }

    async fn label_pr(&self, _number: u64, _labels: &[String]) -> Result<()> {
        Ok(())
    }
                ...
                if let Some(comment) = thread["comments"].as_array().and_then(|c| c.first()) {
                    ...
                    if content.contains(marker) {
                        if let Some(comment_id) = comment["id"].as_u64() {
                            ...
                            self.patch(&patch_url, &serde_json::json!({ "content": body }))
                                .await?;
                            return Ok(());
                        }
                    }
